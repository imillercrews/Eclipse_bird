...
# get average for non-eclipse days  
  ...
  data.filter.sec.sum %>% 
    ...
    group_by(eclipse,
             ...
             Min.eclipse) %>% 
    summarise(Avg.total = mean(Sum.total),
              Min.total = min(Sum.total),
              Max.total = max(Sum.total)) %>% 
    ggplot() +
    annotate("rect",
             xmin = 54295/3600,
             xmax = 54535/3600,
    ...
    # geom_vline(xintercept = 74580/3600,
    #            linetype = 'dashed') +
    geom_ribbon(aes(x = (Min.group+30)/3600, # add 120 seconds to center it
                    ymin = Min.total,
                    ymax = Max.total,
                    group = eclipse),
                linewidth = 1,
    ...
    #      max(data.filter.sec.sum$Min.group)/3600) +
    # ggtitle("All birds: 1 min sum") +
    scale_color_manual(values = c("Control Day" = 'grey50',
                                  "Eclipse" = '#00C58F'),
                       name = 'Day') +
    annotate("rect",
             xmin = (53575)/3600, #-12
    ...
    annotate("rect",
             ...
             fill = '#FFC000') +
    ylim(0,200)
  ggsave('figures/paper/All birds calls moving sum 1 min afternoon average variation paper.pdf',
         height = 3.5,
         width = 6.5,
         units = 'in')
...
for (i in unique(data.filter.sec.sum$Common.Name)) {
  ...
  ggsave(paste0('figures/paper/species/',
                i, 
                ' calls moving sum 1 min.pdf'))
